...
#  提取注意事项
1. **忽略** 含“合计”“汇总”“累计”等关键词的行。
2. 表头/表尾出现的公共字段（如“购电月份”“供电单位”）应自动继承到所有记录。
3. 时间字段统一标准化为 `YYYYMM`，需要同时提取出start_time和end_time，如果发现只能提取其中一个值，则认为两个字段的值相同。
4. generation_account 与 transaction_id 易混淆，必须根据 **排除关键词** 判断归属。
5. 如无法确定字段值，填 `null`。
