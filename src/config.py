...
#  任务说明：
- 根据语义理解，找出每个字段最可能对应的列名，并给出其列号，如果找不到，该字段列号为 null。
- 对于编号类字段generation_account和transaction_id，这两个字段容易混淆，要注意**排除关键词**信息，不要将列名归于错误的字段
- 针对每个字段，从识别到的列中提取前5行数据，并用该字段对应的**值类型**要求进行验证,如果发现大量不符合**值类型**要求的值，该字段列号识别错误，应该为null。
- 输出一个包含所有字段对应列号的Json。

---

# 重要规则
- 一定不要返回**用电户号**所在的列号，不要将**用电户号**和generation_account和transaction_id混淆。

---

#  输出格式（Json）（严格遵循）
- 输出一个Json对象，记录每个字段所在的列号,列号用数字表示（如2表示列号2,0表示列号0），未找到则返回null：
```json
{{output}}
```
...
#  提取注意事项
1. **所有提取的值必须直接来自excel表格，不能通过数学计算得到，比如 grid_price字段如果原表单中没有，不可利用表单中的数据计算得到**。
