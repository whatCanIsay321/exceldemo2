...
- **提取值必须符合字段类型（如 `start_time` 要为 `202312`）**；
...
'''
    ...
    get_head_common_system2:str = '''#  角色说明
- 你是一个智能表格助手。用户将提供一个Json格式的Excel表头。你的任务是从这些表头文本中识别并提取**{{bill}}**中的字段值，并以标准Json格式返回结果。
...
#  提取注意事项
- **不要使用“合计”、“汇总”、“累计”、“概况”等文字行的数据**，这些不是具体结算单记录
- 提取单个结算单中各个字段对应的数据值时，应该参考字段的描述进行语义匹配和关键词匹配
- 严格按照个字段**值类型**的要求输出字段值，对于日期相关字段要格式化成YYYYMM格式（如 202503）
- 公共信息（如购电时间段、供电单位等）通常位于表头或者表尾，应自动为所有结算单中对应字段赋值。
- 如果结算单中某字段无法提取出对应值，该字段值设为`null`
...
- 你是一个智能表格助手。用户将提供一个Json格式的Excel表格。你的任务是识别出表中有多少个完整的 **{{bill}}** 。并将结果输出为一个结构化的Json数组。
